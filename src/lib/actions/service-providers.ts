...
// Cache key for service provider directory (global, not per-practice)
...
// Ensure user is SUPER_ADMIN for write operations
async function ensureSuperAdmin() {
  const { user, practice } = await ensureUserAndPractice();
  if (!practice || !user) throw new Error("Not authenticated");
  if (user.role !== "SUPER_ADMIN") {
    throw new Error("Only platform administrators can manage service providers");
  }
...
export async function getServiceProviderDirectory() {
  const { user } = await ensureUserAndPractice();
  if (!user) return null;
...
export async function getServiceProviderCategories() {
  const { user } = await ensureUserAndPractice();
  if (!user) return [];

  await ensureDefaultCategories();

  return prisma.serviceProviderCategory.findMany({
    orderBy: { sortOrder: "asc" },
    select: {
      id: true,
      name: true,
...
export async function createServiceProvider(data: CreateServiceProviderInput) {
...
export async function updateServiceProvider(id: string, data: CreateServiceProviderInput) {
  await ensureSuperAdmin();
